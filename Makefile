...
generate-in-docker:
	docker run --rm -it -v $(PWD):/go/src/github.com/gardener/gardener-extension-registry-cache golang:1.22.2 \
...
extension-up: $(SKAFFOLD) $(KIND) $(HELM) $(KUBECTL) $(YQ)
	@LD_FLAGS=$(LD_FLAGS) $(SKAFFOLD) run

extension-dev: $(SKAFFOLD) $(HELM) $(KUBECTL) $(YQ)
